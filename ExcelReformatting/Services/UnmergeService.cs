...
namespace ExcelReformatting.Service
{
    ...
    public class UnmergeService : IReadService
    {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream)) // "using" declaration allows for the package to only be used with in a limited scope
                {
                    ...
                    // the file will be read as long as the cell doesn't have a white space or null value 
                    {
                        ...
                        Client c = new Client();
                        /*  a  */
                        c.wfid = MergedCellvalue(ws, row, col);
                        /*  b  */
                        c.dte = MergedCellvalue(ws, row, col + 1);

                        c.dte = reformat_date(c.dte);
                        /*  c  */
                        c.c_n = MergedCellvalue(ws, row, col + 2);
                        /*  d  */
                        c.a_i_d = MergedCellvalue(ws, row, col + 3);
                        /*  e  */
                        c.f_n = MergedCellvalue(ws, row, col + 4);
                        /*  f  */
                        c.ph_num = ((ws.Cells[row, col + 5].Value) == null) ? "" : ws.Cells[row, col + 5].Value.ToString();
                        /*  g  */
                        c.inM = ((ws.Cells[row, col + 6].Value) == null) ? "" : ws.Cells[row, col + 6].Value.ToString();
                        /*  h  */
                        c.cntr = ((ws.Cells[row, col + 7].Value) == null) ? "" : ws.Cells[row, col + 7].Value.ToString();
                        /*  i  */
                        c.comp_tpe = ((ws.Cells[row, col + 8].Value) == null) ? "" : ws.Cells[row, col + 8].Value.ToString();
                        /*  j  */
                        c.comp_desc = ((ws.Cells[row, col + 9].Value) == null) ? "" : ws.Cells[row, col + 9].Value.ToString();
                        /*  k  */
                        c.comp_res = ((ws.Cells[row, col + 10].Value) == null) ? "" : ws.Cells[row, col + 10].Value.ToString();
                        /*  l  */
                        c.com = ((ws.Cells[row, col + 11].Value) == null) ? "" : ws.Cells[row, col + 11].Value.ToString();
                        /*  m  */
                        c.Temp = MergedCellvalue(ws, row, col + 12);
                        /*  n  */
                        c.Wfc = MergedCellvalue(ws, row, col + 13);
